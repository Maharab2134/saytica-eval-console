...
const COLUMNS: StatusColumn[] = [
  { status: "pending",     label: "Pending",     color: "border-amber-500/30 bg-amber-500/5" },
  { status: "in_progress", label: "In Progress", color: "border-blue-500/30 bg-blue-500/5"  },
...
export function TaskBoard({ tasks, onUpdate, isUpdating, isLoading }: TaskBoardProps) {
  if (isLoading) {
    return (
      ...
      {COLUMNS.map(col => {
        ...
        return (
          <motion.div
            ...
            className={cn("rounded-xl border p-3 space-y-3 min-h-[200px]", col.color)}
            role="group"
