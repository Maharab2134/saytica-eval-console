...
export function normalizeModel(raw: RawModel): NormalizedModel {
  return {
    ...
    accuracy:
      typeof raw.accuracy === "number" && isFinite(raw.accuracy)
        ? raw.accuracy
        : null,
    latencyMs:
      typeof raw.latencyMs === "number" &&
      isFinite(raw.latencyMs) &&
      raw.latencyMs < 9000
        ? raw.latencyMs
        : raw.latencyMs === 9999
          ? 9999
          : raw.latencyMs,
    costPer1k:
      typeof raw.costPer1k === "number" && isFinite(raw.costPer1k)
        ? raw.costPer1k
        : null,
...
export function normalizeModels(raws: RawModel[]): NormalizedModel[] {
  return raws
    .filter(
      (r): r is RawModel =>
        r !== null && typeof r === "object" && Boolean(r.id),
    )
    .map(normalizeModel);
}
...
export function normalizeStatus(raw: string | null | undefined): TaskStatus {
  ...
  switch (normalized) {
    ...
    case "completed":
      return "done";
    default:
      return VALID_STATUSES.includes(normalized as TaskStatus)
...
export function normalizeTask(raw: RawTask): NormalizedTask {
  return {
    ...
    assignedTo:
      raw.assignedTo && typeof raw.assignedTo === "string"
        ? raw.assignedTo
        : null,
...
export function normalizeTasks(raws: RawTask[]): NormalizedTask[] {
  return raws
    .filter(
      (r): r is RawTask => r !== null && typeof r === "object" && Boolean(r.id),
    )
    .map(normalizeTask);
}
